	...
	Validform.util={
		...
		showmsg:function(msg,type,o,show){
			...
			*/
			...
			if(type==1 || show=="alwaysshow" && type!=4){
				msgobj.find(".Validform_info").html(msg);
			}

			if(type==1 && show!="hide" || show=="alwaysshow" && type!=4){
				msghidden=false;
		...
		check:function(curform,brothers,subpost,bool){
			...
			*/
			...
			if(inputval==this.validform_lastval && !$(this).attr("recheck") || $(this).data("dataIgnore")==="dataIgnore"){
				return true;
			}
			
			//dragonfly=true时，值为空不做验证，但验证不通过;
			if(settings.dragonfly && !$(this).data("cked") && Validform.util.isEmpty.call($(this),inputval) ){
				return false;
			}
			...
			if(!flag.passed){
				...
				if(!bool){
					...
					Validform.util.showmsg.call($this,flag.info,settings.tiptype,{obj:$(this),type:flag.type,sweep:settings.tipSweep},"hide"); //当tiptype=1的情况下，传入"hide"则让提示框不弹出,tiptype=2、3的情况下附加参数"hide"不起作用;
				}
				return false;
			}else{
				if($(this).attr("ajaxurl") && !bool && $(this).attr("ignore")!="ignore"){
					var inputobj=$(this);
					...
					_this[0].validform_ajax=$.ajax({
						...
						cache:false,
						url: ajaxurl,
						data: Validform.util.getQuery.call(win,ajaxurl)+"param="+inputval+"&name="+$(this).attr("name"),
						dataType: "text",
						success: function(s){
							if($.trim(s)=="y"){
		...
		submitForm:function(settings,flg,ajaxPost,sync){
			...
			*/
			...
			curform.find("[datatype]").each(function(){
				...
				if($(this).attr("ajaxurl") && $(this).attr("ignore")!="ignore"){
			...
			if(flag){
				...
				if(settings.ajaxPost || ajaxPost==="ajaxPost"){
					Validform.util.showmsg.call(curform,curform.data("tipmsg").p||tipmsg.p,settings.tiptype,{obj:curform,type:1,sweep:settings.tipSweep},"alwaysshow");//传入"alwaysshow"则让提示框不管当前tiptye为1还是2或3都弹出;

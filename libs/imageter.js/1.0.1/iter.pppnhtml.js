...
var appUrl="";
var scriptUrlSub = "iter.pppnhtml.js";
	var closeWidth = 40;
	var closeHeight = 13;
...
ISXAd.prototype = {
	...
	createApps:function(){
		...
		if (creative.type == 0) { // image
			...
			if(creative.material && creative.material.toLowerCase().indexOf('http://')==0){	
				
				if (typeof refurl != "undefined" && creative.material.toLowerCase().indexOf('(url)')>0) { 
					//var eccurl = UrlEncode1(refurl);
					//alert(eccurl);
					creative.material = creative.material.replace('(url)',refurl);
				}
				var frame = '<iframe src="' + creative.material + '" scrolling="no" height="' + creative.h + '" width="' + creative.w + '" frameborder="0" border="0" marginwidth="0" marginheight="0"></iframe>';
				contentHtml += frame;
				contentWrapper.innerHTML =  contentHtml; 
			}
			...
			if(creative.code && creative.code.toLowerCase().indexOf('http://')==0){	
				...
				if (typeof refurl != "undefined" && creative.code.toLowerCase().indexOf('(url)')>0) { 
					alert(UrlEncode(refurl));
					creative.code = creative.code.replace('(url)',UrlEncode(refurl));
				}

...
var appUrl="";
var scriptUrlSub = "iter.pppbmh.js";
	var closeWidth = 40;
	var closeHeight = 13;  
	var appWidth = 0;
	var appHeight = 0;
	var appLoc = 11;
	var smallMillisecond = 15000;		
	var smallWidth = 100;
	var smallHeight = 100;	 
	var smallcan = false;
var scriptUrlHeader="";
...
var ISXAd = function(data) {	
	...
	this.contents = null;
	this.closes = null; 
	this.smallcontents = null;
	this.smallcloses = null; 
...
ISXAd.prototype = {
	...
	createApps:function(){
		...
		var smallContentWrapper = document.createElement("div");
		var smallCloseWrapper = document.createElement("div"); 
		...
		smallContentWrapper.id = "isx-contents-wrapper-ps"; 
		smallCloseWrapper.id= "isx-close-wrapper-ps";	
		appWidth = creative.w;
		appHeight = creative.h;
		appLoc = configp.app_location;
		isx.container.appendChild(contentWrapper);
		isx.container.appendChild(smallContentWrapper);
		//create ad
		if (creative.type == 0) { // image
			redUrl = (creative.click || '');
				...
				smallcontentHtml = "<img src='" + creative.material + "' alt=''  width='" + smallWidth + "' height='" + smallHeight+ "' border='0px'/>";
			    smallContentWrapper.innerHTML =  smallcontentHtml;
				smallcan = true;
			...
			contentHtml += '<object  name="afg-adloader" id="afg-adloader" width="' + creative.w + '" height="' + creative.h + '"  align="middle" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=10,0,0,0" classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000">';
			...
			smallcontentHtml = '<object  name="afg-adloaders" id="afg-adloaders" width="' + smallWidth + '" height="' + smallHeight + '"  align="middle" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=10,0,0,0" classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000">';
			smallcontentHtml += '<param value="always" name="allowScriptAccess"/><param value="' + creative.material + '" name="movie"/><param value="high" name="quality"/><param value="opaque" name="wmode"/><param value="high" name="quality"><param value="#F1F1F1" name="bgcolor"/>';
			smallcontentHtml += '<embed width="' +smallWidth + '" height="' + smallHeight + '" align="middle" pluginspage="http://www.adobe.com/go/getflashplayer"  type="application/x-shockwave-flash" allowscriptaccess="always" wmode="opaque"  bgcolor="#F1F1F1" quality="high" src="' + creative.material + '"></object>';
			smallContentWrapper.innerHTML =  smallcontentHtml;
			smallcan = true;
		...
		contentWrapper.style.cssText="display:block;position:fixed;"+_this.getLocClass(appWidth+20,appHeight+20)+";z-index:2147483646;float:right;margin-left:0px !important;text-align:left;overflow:hidden;border:0px solid #BBB;"+contentLenStr;
       
		contentLenStr="width:"+ smallWidth + "px;" +"height:"+ smallHeight + "px;"; 
		smallContentWrapper.style.cssText="display:none;position:fixed;"+_this.getLocClass(smallWidth+20,smallHeight+20)+";z-index:2147483646;float:right;margin-left:0px !important;text-align:left;overflow:hidden;border:0px solid #BBB;"+contentLenStr;
        
		//create close		
		closeHtml = '<a title="关闭" href="javascript:;"></a>';
		closeWrapper.innerHTML = closeHtml;  
		var closeCssText="display:block;background-color:#F1F3F5;position:absolute;right:3px;bottom:"+(creative.h-17)+"px;filter:alpha(Opacity=90);-moz-opacity:0.9;opacity: 0.9;background:url("+allUrlHeader+"/images/isx-close.png) no-repeat;color: #555555;font:normal 10px/12px Arial,Microsoft Yahei;";
		...
		closeHtml = '&nbsp;&nbsp;';
		smallCloseWrapper.innerHTML = closeHtml;  
		closeCssText="display:none;background-color:#F1F3F5;position:absolute;right:3px;bottom:"+(smallHeight-16)+"px;filter:alpha(Opacity=90);-moz-opacity:0.9;opacity: 0.9;background:url("+allUrlHeader+"/images/isx-closes.png) no-repeat;color: #555555;font:normal 10px/12px Arial,Microsoft Yahei;";
            closeCssText = closeCssText+"text-align: center;z-index:2147483646;border: 0px solid #BBB;";  
		    closeCssText= closeCssText + "width:13px;" +"height:13px;";
		smallCloseWrapper.style.cssText=closeCssText; 
		isx.container.appendChild(smallCloseWrapper); 

		_this.contents = contentWrapper;
		_this.closes = closeWrapper; 
		_this.smallcontents = smallContentWrapper;
		_this.smallcloses = smallCloseWrapper; 
         
		_this.bindClosesEvents();
		_this.bindAdsEvents(); 
		_this.autoZoomSmall();
		_this.bindSmallEvents();
	},
	...
	closeApp: function() {
		...
		_this.smallcontents.style.display = "none";
		_this.smallcloses.style.display = "none";	 
		...
		clearTimeout(_this.zoomTimerId);
	},
	bindSmallEvents: function() {
		var _this = this;
		ev.bind(_this.smallcontents, 'mouseover', function() {
			  _this.autoShowNormal();
		});
		ev.bind(_this.smallcloses, 'mouseover', function() {
			  _this.autoShowNormal();
		});
	},
	bindClosesEvents: function() {
		...
		ev.bind(_this.closes, 'mouseover', function() {
			//document.getElementById('test1').innerHTML="bindClosesEvents mouseover";
			clearTimeout(_this.zoomTimerId);
		});
		/*ev.bind(_this.closes, 'mouseout', function() {
			document.getElementById('test1').innerHTML="bindClosesEvents mouseout";
			_this.autoZoomSmall();
		});*/
	...
	bindAdsEvents:function(){
		...
		ev.bind(_this.contents, 'mouseover', function() {
			//document.getElementById('test1').innerHTML="bindAdsEvents mouseover";
			clearTimeout(_this.zoomTimerId);
		});
		ev.bind(_this.contents, 'mouseout', function() {
			//document.getElementById('test1').innerHTML="bindAdsEvents mouseout";
			_this.autoZoomSmall();
		});		
	},
	...
	autoZoomSmall:function(){	
		var _this = this;	
		_this.zoomTimerId = setTimeout(function() {
			_this.contents.style.display = "none";
			_this.closes.style.display = "none";
			_this.smallcontents.style.display = "block";
			_this.smallcloses.style.display = "block";	
		}, smallMillisecond);
	},
	autoShowNormal:function(){
		var _this = this;	
		_this.contents.style.display = "block";
		_this.closes.style.display = "block";
		_this.smallcontents.style.display = "none";
		_this.smallcloses.style.display = "none";	
	}
};
